...
- 코드 기준: `ac34a74` 이후 현재 작업 트리
- MCP: 0.7.1, Streamable HTTP `/mcp/`, 도구 13개, 공개 생성 capability 5개
- 자동 회귀: `unittest` 141개 통과
- 운영 자산 audit: catalog row 1,281개, active Game UI 그룹 5개
- 정합성: `missing_files=0`, `missing_metadata=0`, `missing_group_files=0`
- 실제 클라이언트: Codex 앱·IDE 경로 검증 완료, Claude Code 사내 구독 계정 검증 예정
- 온보딩: `/mcp-connect` Edge E2E와 390px 반응형 검증 완료
...
| 참고 이미지 편집 | 완료 | MCP 첨부·소유 자산 편집, 결과 조회 |
...
| Claude Code 화면 표시 | 미검증 | 실제 사내 구독 계정 필요 |
| 장시간·다사용자 부하 | 미검증 | 단일 개발자 환경 밖의 운영 표본 필요 |

현재 합의한 1차 기능 범위에는 사내 파일럿을 막는 기능 누락이 없습니다. 남은 항목은
...
후처리를 한 차례 생략한 사례도 있었습니다. 이를 반영한 0.7.1 응답 hardening과 기존 작업의
...
2. 동료의 실제 Claude Code 계정으로 연결·읽기·생성 승인·결과 표시를 단계별 검증
3. 완전 백업 목적지를 별도 볼륨 또는 NAS로 확정하고 예약 작업·복구 훈련 수행
4. actual cost 표본을 모아 모델·크기·품질별 보수적 사전 비용표 결정
5. 여러 사용자·장시간 운영에서 외부 API 동시성과 로컬 RMBG 단일 큐 관찰
6. 필요성이 확인되면 IP workspace retirement와 owner generation 기능 설계
